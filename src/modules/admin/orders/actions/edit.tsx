...
import type { Order, Statuses } from '@/store/api/orders/orders.types'
...
export const EditOrderModal = ({ order }: EditOrderModalProps) => {
    ...
    const [editOrder, { isLoading }] = usePatchOrdersMutation()

    const form = useCustomForm(orderSchema, {
        recipient: {
            id: order?.creator?.id.toString(),
            name: order?.creator?.first_name + ' ' + order?.creator?.last_name
        },
        address: {
            id: order?.address?.id.toString(),
            name: order?.address?.city + ', ' + order?.address?.street
        },
    ...
    const handleEditOrder = (data: OrderFormValues) => {
        const payload = {
            ...data,
