...
import {
    InCartIndicator,
    OrdersProductStatusesCards
} from './orders-product-statuses-cards'
...
export const OrdersProductRowCard = ({
    isOpen,
    onToggle,
    shopProduct,
    orderItems,
    ...
    const isOrderItemInCart = shopProduct.stocks.some((stock) =>
        orderItems.some((orderItem) => orderItem.id === stock.id)
    )
    ...
    return (
        <Collapsible
            ...
            className={cn(
                'relative rounded-md border-2 border-secondary p-2 transition-colors data-[state=open]:border-primary data-[state=open]:bg-primary',
                isVisiblie ? '' : 'opacity-65'
            )}
        >
            {isOrderItemInCart ? <InCartIndicator /> : null}
